...
static esp_err_t http_server_setRGB_json_handler(httpd_req_t *req)
{
	...
	cJSON *row_json = cJSON_GetObjectItem(root, "row");
	...
	if (row_json == NULL ||value_R_json == NULL || value_G_json == NULL || value_B_json == NULL || !cJSON_IsString(row_json) || !cJSON_IsString(value_R_json) || !cJSON_IsString(value_G_json) | !cJSON_IsString(value_B_json))
	{
		cJSON_Delete(root);
		// Missing or invalid JSON fields
		// httpd_resp_send_err(req, HTTP_STATUS_BAD_REQUEST, "Missing or invalid JSON data fields");
